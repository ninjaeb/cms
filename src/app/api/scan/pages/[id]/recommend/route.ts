...
import { fetchAndParsePage } from "@/lib/htmlScan";
...
export async function POST(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const pageScan = await prisma.pageScan.findUnique({ where: { id } });
  if (!pageScan) return NextResponse.json({ error: "Not found" }, { status: 404 });

  let signals;
  try {
    signals = await fetchAndParsePage(pageScan.sourceUrl);
  } catch (err) {
    return NextResponse.json(
      { error: `Could not re-fetch the page: ${err instanceof Error ? err.message : String(err)}` },
      { status: 400 },
    );
  }
